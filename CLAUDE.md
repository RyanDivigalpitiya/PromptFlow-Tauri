...
- **UI driving** (`scripts/*.swift`, CGEvent-based): `clickwin.swift dx dy`,
  ...
  A CGEvent hover/click also only lands while the dev window is FRONTMOST (AppKit routes
  mouse-moved to the key window) — so it steals the user's focus, and a hover that
...
- **Headless visual QA** (`scripts/qa.mjs`, Playwright WebKit = the WKWebView's engine):
  ...
  (the completion, parent-glyph, focus-pane and prompt-fold sections each do) — the sweep
  tests anchor on "the last row" and on the
  ...
  `cargo test`'s job, and the two halves meet at the command name + args. When the RESULT
  is what's in question, the section stages it through `__PF_QA_DELTA` with the literal a
  `cargo test` pins over the same tree (the prompt fold does this) — the two suites then
  meet at that string, and the rendering half gets checked in the engine that has to hold
  it: a fold's indent is LEADING WHITESPACE, which lives or dies on `white-space:
  pre-wrap`, and no Rust test can see that. Deterministic
...
- **Block styling / folding**: ⌘B over a selection is `toggle_bold_block` — a UNIFORM toggle (any member not bold end-to-end ⇒ bold them all), the same all-or-nothing shape `toggle_completed_block` has, because per-member toggling turns a mixed selection into its own inverse and reads as nothing happening. Empty-text members are SKIPPED, not given a `[0,0]` run, or the block could never be un-bolded. ⌘3 over two-or-more NON-prompt members is `merge_into_prompt`, which FOLDS the block into its first member: that node becomes a `promptDraft` whose text is the block as an INDENTED `"- "` list, everything folded is deleted, and it is ONE transaction, so one ⌘Z puts the block back. **The block is not just the members.** A selection is a sibling range with every member's SUBTREE tinted, so what the user swept spans levels — and each descendant is its own bullet, two spaces deeper per level (`fold_lines` walks members at depth 0, pre-order, and the composition prefixes `INDENT.repeat(depth)`). That is the bullet-and-indent shape `copySubtree`/⌘C already write to the clipboard (`subtreeLines`), and the two agree exactly on any plain tree; they diverge only where the fold has to do more — it indents a node's OWN newlines to the bullet's text column, which the clipboard copy leaves flush, and it folds no notes, which the copy does write. Folding used to take the members' text only and drop every descendant on the floor with the nodes ("no non-arbitrary home for unmentioned children"); they are all mentioned now, which is why the HEAD's own children are deleted too — they are in its text, and leaving them nested under the new prompt would show every folded line twice. It keeps its note and position, but NOT its completion — the one place the fold diverges from `set_kind`/`set_kind_block`, which leave the flag alone. A kind change keeps the node; a fold makes a NEW document out of many, "done" described the one that no longer exists in that form, it would be arbitrary (whichever member was swept first decides), and it can be false on its face now that the merged text holds every unfinished line in the block — which is also a struck-through dimmed panel for the caller to focus you into. Ageing out into the auto-archive as a childless completed node is then one thing less that can happen to it, but that state is reachable plenty of other ways (delete a completed parent's children by any means), so it is not a fix for the sweep. A node's own multi-line text (⇧Enter) is folded line by line, later lines indented to the bullet's TEXT column, so a wrapped thought stays visibly part of its bullet. Style runs are carried across: each LINE lands at a known offset in the merged string, so its runs CLIP and shift by exactly that much (`push_clipped_ranges`, the mirror of `split_ranges` — it has to clip as well as shift because a member's lines no longer land as one piece, and a run spanning a newline comes out as two with the indent between them unstyled). Offsets are UTF-16 units (`utf16_len`), never `String::len()`. Notes are NOT folded in — a note is a second field rather than a line of the outline, and the head's own note survives as a note, so folding the others' in would put the same content in two different places depending on which member was swept first. Dividers are skipped for the same reason ⌘1/2/3 never converts one, so a top-level one neither counts toward the two nor gets folded, and a nested one contributes no line before going with its parent. The two-member floor counts MEMBERS, not folded lines: ⌘3 on ONE node, children or not, stays the ordinary "make this a prompt" conversion and must never silently eat its subtree. A range that already CONTAINS a prompt falls through to `set_kind_block` — the user is converting a mixed selection, not collecting one — and that test is member-level like the count, since a draft buried inside a member's subtree is content being collected, not a signal. `MutationOut.new_node` carries the head so the caller focuses it; the selection is dropped, since one surviving node is nothing left to act on.
...
- **Chrome-less title bar is a three-part coupling — change all or none**: `.topbar`
  ...
  eyeballing.
- **The glass is `windowEffects.state: "active"`, and BOTH window paths must say so** — `tauri.conf.json` (main) and the `.effects(...)` call in `lib.rs`'s ⌘N builder, where the field is `Some(WindowEffectState::Active)` and `None` is NOT a neutral default (window-vibrancy simply doesn't set the property, leaving NSVisualEffectView at its own default, which is `followsWindowActiveState` — the same wrong value the config used to spell out). Following the active state swaps the `underWindowBackground` material for a flat fill the instant the window stops being key, so an unfocused window read as OPAQUE while the focused one was translucent (shipped bug, fixed). Nothing in CSS was involved: the app's wash `--bg-tint` is a constant `rgba(10,10,14,0.45)` over a `transparent: true` window, so the blur behind it is the only variable, and it is macOS's to vary. `Active` is also what a multi-window app wants by definition — every ⌘N peer is a full peer, and only one of them can be key. **Window effects are NOT verifiable under `scripts/dev.sh`**: `tauri dev` runs the unbundled `target/debug/promptflow-tauri`, which shows no blur in EITHER focus state, so a dev window is opaque glass-less in both and proves nothing either way — check them on `scripts/build.sh`'s bundle. Nor headlessly (qa.mjs has no window at all). The measurement that settles it: launch the release binary with an isolated `PROMPTFLOW_STORE`, `screencapture -R` the same rect key and non-key, and diff — the body should come back byte-identical (mean abs channel diff 0.0) with a real gradient in the empty area below the last row (a flat inactive fill has none). Match the window by the launched child's PID, never by owner name: the release bundle's owner is "PromptFlow", identical to the user's real app.
...
- **Row interaction map** (matches the original): click bullet/prompt-line = drill;
  ...
  `neighborNode` (controller.ts) skips `line` rows, since a divider renders no editor and
  focusing one dropped the caret out of the outline entirely (shipped bug, fixed; it
  covers ↑/↓, backspace-on-empty and ⌘Enter's move-to-next alike).
- **Trailing cluster placement is per KIND** (`TrailingCluster`'s `layout` prop picks the
  arrangement; WHICH buttons exist stays per-kind). `inline` = one run after the text.
  `prompt` = a column down the panel's trailing edge (`align-self: stretch` +
  `justify-content: space-between`), chevron + zoom + ⋯ at its top edge, "+" at its
  bottom, `align-items: flex-start` so the lone "+" hugs the panel instead of hanging
  under the ⋯; `.cluster-line` is 1.2em, NOT the 1.35em row line box — two of those
  overhang the panel's 2.5em min-height. A DIVIDER's actions sit BETWEEN the handle and
  the rule in a clip that is 0-wide at rest, so an unhovered divider is only its line,
  still reaching the row's leading edge; `.line-actions` animates `grid-template-columns:
  0fr ↔ 1fr` (the horizontal twin of the focus pane's auto-height trick — WebKit
  interpolates it), and the `flex: 1` rule gives up exactly that width, so the line
  shrinks from its LEFT end. Two traps: `overflow: hidden` + `min-width: 0` on the clipped
  child is load-bearing TWICE (it zeroes the grid item's automatic minimum size, without
  which the track never reaches 0, and it clips HIT-TESTING so a collapsed button can't be
...
- **The outbox** (`outbox` table, same SQLite file as the nodes) is written INSIDE the
  ...
  undone deletion is).
  Three rules added after the 2026-08-12 incident (each pinned by a test; the full chain lives in the plan doc's §10.4): (1) **the queue drains parents before children ACROSS push batches** — `enqueue_all_live` seeds in pre-order with increasing `queued_at`, and `plan_push` (sync.rs) defers any upsert whose parent is pending behind the batch and sorts what ships parents-first, because the hub's own reorder sees one request at a time and its orphan repair flattens a stranded child PERMANENTLY (the repair keeps the structure clock, so the flat version wins every later tie — 167 nodes went that way during the first seed); (2) **deletes journal tombstones while sync is configured-but-OFF** (`journal_deletes` — the URL setting is the configuration mark, deliberately not the cursor, which a configure→seed→disable sequence never writes), so re-enabling reconciles what stopped existing instead of resurrecting it — an import-replace while off used to leave the hub holding the entire old outline alive; (3) **the hub's mass-delete tripwire weighs what would actually die** — cascade descendants count, a device's deletes applied in the last ten minutes count, and one confirmation covers the device for that window (`devices.confirmed_until`) rather than one request.
