...
> **IMPLEMENTATION STATUS, 2026-08-12.** Phases 0–5 are **built and green**, and the hub is **DEPLOYED**: running on the mini since 2026-08-12, healthy through the tunnel, `iss` pinned to `https://throbbing-math-6ac0.cloudflareaccess.com` (§10.3 step A complete; the newsyslog sudo is still optional-pending). What is left: point the Mac at it (step B), bootstrap the iPad (step C), watch for a week (step D). See §10 for the handover checklist and for the places the implementation deviated from this document (each with its reason — including the ninth bug, found on deploy day in the one path no test had executed).
