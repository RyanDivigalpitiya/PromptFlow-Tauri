...
> **IMPLEMENTATION STATUS, 2026-08-12.** Phases 0–5 are **built and green**, the hub is **DEPLOYED** (running on the mini, healthy through the tunnel, `iss` pinned to `https://throbbing-math-6ac0.cloudflareaccess.com`), and the Mac is **LIVE-SYNCING**: §10.3 steps A and B completed 2026-08-12 — the seed pushed all 765 nodes losslessly (hub count = store count), typing/second-window/delete checks all verified on the hub (tombstones propagate, cursor advances). Still optional-pending: the newsyslog sudo. What is left: bootstrap the iPad (step C), watch for a week (step D). See §10 for the handover checklist and for the places the implementation deviated from this document (each with its reason — including the ninth bug, found on deploy day in the one path no test had executed).
...
**Deploy day (2026-08-12) found a ninth, in the one code path no test had ever executed.** `jsonwebtoken = "11"` ships NO signature backend by default (`default = ["use_pem"]` only) — it compiles clean and panics at the first `decode()` with "Could not automatically determine the process-level CryptoProvider". Every suite runs with `require_access = false`, so the first RS256 verification ever performed was the hub's first authenticated request through the tunnel, which died as a 502 while `launchctl` reported the process healthy. Fixed by pinning `features = ["rust_crypto"]`; pinned by a real sign-and-verify RS256 round-trip test in `auth.rs` (mutation-tested: dropping the feature fails it at the exact line from the mini's log). The general lesson is the plan's own §7 rule turned on itself: the Access-JWT verify path was "tested" only in the sense that a mock had exercised the surrounding middleware — a dependency whose crypto is feature-gated is untested until real bytes have gone through it.
