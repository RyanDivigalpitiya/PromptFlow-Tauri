 ...
 *   cross-fading.
 * - `null` — no animation; the glyph snaps, exactly as it did before any of this. A
 *   prompt and a divider are here because they have no designed transition YET, not
 *   because one is impossible.
 */
export type KindMorphStyle = "scale";
...
export function kindMorphStyle(
  ...
  if (from === "promptDraft" || to === "promptDraft") return null;
  return "scale";
}
